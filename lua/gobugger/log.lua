---gobugger.log — single-file logging surface for the plugin.
---
---Per ADR 0021 §6 (the "wrapper rule"), every auto-family plugin
---owns exactly one `lua/<plugin>/log.lua` that delegates to
---`auto-core.log`. Feature code in gobugger calls THIS module;
---feature code MUST NOT `require("auto-core").log` directly.
---
---gobugger's pre-ADR-0021 pattern had two local `notify()` helpers
---(in run.lua and launch.lua) plus four bare `vim.notify(...)` call
---sites scattered across ui.lua / init.lua / errors.lua. The Phase 2
---sweep routes both helpers through this wrapper and replaces the
---bare calls with namespaced level functions.
---
---@module 'gobugger.log'
...
-- Pre-auto-core fallback. Honors the v0.x `notify_title` config when
-- available so users without auto-core keep their toast title.
local function _legacy_notify(component, msg, level)
  local title
  local ok, cfg = pcall(require, "gobugger.config")
  ...
  local prefix = (type(component) == "string" and component ~= "")
    and ("[" .. NS .. "." .. component .. "] ")
    or  ("[" .. NS .. "] ")
  vim.notify(prefix .. tostring(msg), level, title and { title = title } or nil)
end
